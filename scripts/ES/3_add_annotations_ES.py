...
def filter_by_contamination(
    ...
    data_export.export(f"{output_dir}/sample_contamination_ES{subset_name}.tsv")
...
def export_simplified_variants(input_ht: hl.Table, output_dir: str, subset_name: str) -> None:
    ...
    reduced_ht.export(f"{output_dir}/reduced_annotations_ES{subset_name}.txt")
...
def change_to_grch38_chrm(input_mt: hl.MatrixTable) -> None:
    ...
    :param input_mt: MatrixTable
    :return: Matrible with GRCh38 reference genome subsetted to just chrM (so that extraneous contigs will be excluded from VCF output)
    """
...
def main(args):  # noqa: D103
    ...
    mt = mt.checkpoint(f"{temp_dir}/prior_to_vep_ES.mt", overwrite=args.overwrite)
    ...
    annotated_mt_path = generate_output_paths(
        output_dir, "annotated_combined_ES", subset_name, "mt"
    )
    sites_ht_path = generate_output_paths(
        output_dir, "combined_sites_only_ES", subset_name, "ht"
    )
    sites_txt_path = generate_output_paths(
        output_dir, "combined_sites_only_ES", subset_name, "txt"
    )
    sites_vcf_path = generate_output_paths(
        output_dir, "combined_sites_only_ES", subset_name, "vcf.bgz"
    )
    samples_txt_path = generate_output_paths(
        output_dir, "sample_annotations_ES", subset_name, "txt"
    )
    samples_vcf_path = generate_output_paths(
        output_dir, "sample_vcf_ES", subset_name, "vcf.bgz"
    )
    ...
    mt = mt.checkpoint(
        f"{temp_dir}/prior_to_filter_genotypes_ES.mt", overwrite=args.overwrite
    )
