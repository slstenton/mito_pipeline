...
def filter_by_copy_number(
    input_mt: hl.MatrixTable, keep_all_samples: bool = True
) -> hl.MatrixTable:
    """
...
def filter_by_contamination(
    ...
    input_mt = input_mt.annotate_cols(
        ...
        & ((hl.is_missing(input_mt.freemix_percentage) | input_mt.freemix_percentage < 2))
    ...
    data_export.export(f"{output_dir}/sample_contamination_GS{subset_name}.tsv")
...
def generate_expressions(
    ...
    The expressions include AC, AN, AF, filtering allele frequency (FAF) split by homplasmic/heteroplasmic, and haplogroup.
...
def export_simplified_variants(input_ht: hl.Table, output_dir: str, subset_name: str) -> None:
    ...
    reduced_ht.export(f"{output_dir}/reduced_annotations_GS{subset_name}.txt")
...
def main(args):  # noqa: D103
    ...
    mt = mt.checkpoint(f"{temp_dir}/prior_to_vep_GS.mt", overwrite=args.overwrite)
    ...
    annotated_mt_path = generate_output_paths(
        output_dir, "annotated_combined_GS", subset_name, "mt"
    )
    sites_ht_path = generate_output_paths(
        output_dir, "combined_sites_only_GS", subset_name, "ht"
    )
    sites_txt_path = generate_output_paths(
        output_dir, "combined_sites_only_GS", subset_name, "txt"
    )
    sites_vcf_path = generate_output_paths(
        output_dir, "combined_sites_only_GS", subset_name, "vcf.bgz"
    )
    samples_txt_path = generate_output_paths(
        output_dir, "sample_annotations_GS", subset_name, "txt"
    )
    samples_vcf_path = generate_output_paths(
        output_dir, "sample_vcf_GS", subset_name, "vcf.bgz"
    )
    ...
    mt = mt.checkpoint(
        f"{temp_dir}/prior_to_filter_genotypes_GS.mt", overwrite=args.overwrite
    )
