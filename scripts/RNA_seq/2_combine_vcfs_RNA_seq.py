...
def collect_vcf_paths(
    ...
    .. note::
        ...
        - 's': sample name
        - path to the MToolBox VCF output, where name of this column is supplied to the `vcf_col_name` parameter
...
def join_mitochondria_vcfs_into_mt(
    ...
    for sample, vcf_path in vcf_paths.items():
        try:
            recode = {'chrMT':'chrM'}
            mt = hl.import_vcf(vcf_path, reference_genome="GRCh38", force_bgz=True, contig_recoding=recode)
        except Exception as e:
            raise ValueError(
                f"vcf path {vcf_path} does not exist for sample {sample}"
            ) from e

        logger.info("Split multiallelic sites in VCF...")
        bi = mt.filter_rows(hl.len(mt.alleles) == 2)
        bi = bi.annotate_rows(a_index=1, was_split=False)
        bi = bi.annotate_entries(HF = bi.HF[0])
        multi = mt.filter_rows(hl.len(mt.alleles) > 2)
        split = hl.split_multi_hts(multi, permit_shuffle=True)
        split = split.annotate_entries(HF = split.HF[split.a_index - 1])
        mt = split.union_rows(bi)
        # Remove duplicated variants, occur due to same variant on multiple lines of the vcf e.g., chrM:16183A>ACCC and chrM:16182AAC>AACC
        mt = mt.distinct_by_row()
        
        # Because the vcfs are now split, there is only one HF value (no longer an array)
        # Set MQ to NA value
        # Add FT annotation for sample genotype filters (pull these from filters annotations of the single-sample VCFs)
        mt = mt.select_entries("DP", HL=mt.HF)
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "-v",
        "--vcf-col-name",
        help="Name of column in participant data file that contains the path to the VCF output by MToolBox",
        required=True,
    )
    p.add_argument(
        "-a",
