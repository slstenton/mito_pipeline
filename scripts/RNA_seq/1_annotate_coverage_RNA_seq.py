...
def main(args):  # noqa: D103
    ...
    with hl.hadoop_open(input_tsv, "r") as f:
        ...
        for line in f:
            ...
            cov_ht = hl.import_table(base_level_coverage_metrics, delimiter="\t", types={'Position': hl.tint32, 'Cov': hl.tint32})
            cov_ht = cov_ht.select(
                s =  sample,
                chrom = "chrM",
                pos = cov_ht.Position,
                coverage = cov_ht.Cov)
            mt = cov_ht.to_matrix_table(
                col_key=['s'],
                row_key=['chrom', 'pos'],
            )
            mt_list.append(mt)
    ...
    cov_mt = cov_mt.annotate_rows(
        ...
        over_equal_20=hl.float((hl.agg.count_where(cov_mt.coverage > 20) / n_samples)),
        over_equal_100=hl.float((hl.agg.count_where(cov_mt.coverage > 100) / n_samples)),
        over_equal_1000=hl.float((hl.agg.count_where(cov_mt.coverage > 1000) / n_samples)),
    )
    cov_mt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", 
        "--output-ht", help="Name of ht to write output", required=True
    )
    parser.add_argument(
        "-t",
        "--temp-dir",
